...
namespace PEMVIS
{
    internal class Transaksi
    {
        ...
        public Transaksi(string no_transaksi, string no_hp, string nama_pelanggan, string kategori, string jumlah, string harga, string jam, string tanggal_terima, string tanggal_ambil, string kasir)
        {
        ...
        public int insert()
        {
            ...
            MySqlCommand cmd = new MySqlCommand("INSERT INTO transaksi (no_transaksi, no_hp, nama_pelanggan, kategori, jumlah, harga, jam, tanggal_terima, tanggal_ambil, kasir) VALUES(@no_transaksi, @no_hp, @nama_pelanggan, @kategori, @jumlah, @harga, @jam, @tanggal_terima, @tanggal_ambil, @kasir)");
        ...
        public int update()
        {
            int result = 0;
            MySqlConnection connect = new MySqlConnection(conString);
            MySqlCommand cmd = new MySqlCommand("UPDATE transaksi SET no_hp=@no_hp, nama_pelanggan=@nama_pelanggan, kategori=@kategori, jumlah=@jumlah, jam=@jam, tanggal_terima=@tanggal_terima, tanggal_ambil=@tanggal_ambil, kasir=@kasir  WHERE no_transaksi=@no_transaksi", connect);
            cmd.Parameters.AddWithValue("@no_transaksi", no_transaksi);
            cmd.Parameters.AddWithValue("@no_hp", no_hp);
            cmd.Parameters.AddWithValue("@nama_pelanggan", nama_pelanggan);
            cmd.Parameters.AddWithValue("@kategori", kategori);
